<template>
	<view class="content">
		...
		></view>
			...
			{{ requestParam.textLocation }}


			<view>
				
				<text 
				v-for="(item, index) in requestParam.text"
				:class="index <= requestParam.textLocation ? 'text-def text-readed' : 'text-def'"
				>
				{{ item }}
				</text>
			</view>
	...
	export default {
		...
		methods: {
			init() {
				console.log("123");
				var dic = {
					"speechString": this.requestParam.text, //文本
				...
				KJSpeechSynthesizer.init(dic, (res) => {
					...
					if (res.type == "init") {
						...
						this.requestParam.textLocation = res.location;
			...
			willSpeakRangeOfSpeechString(){
				KJSpeechSynthesizer.willSpeakRangeOfSpeechString((res) => {
				});
			},
			speakUtterance() {
